...
// The stats to keep showing after a reply. Fresh, non-empty stats replace the old
// ones; anything else - a failed fetch, a 404 before the race has data, an empty
// payload, all of which arrive as null-ish stats - keeps the previous snapshot.
// A race can lose signal deep in the woods, and a blip must not blank the
// standings to "--"; they refresh on the next successful tick. Before the first
// success there is nothing to keep, so it stays null and the rows read "--".
export function keepLatestStats(previous, incoming) {
  if (incoming != null && typeof incoming === "object" && Object.keys(incoming).length > 0) {
    return incoming;
  }
  return previous;
}
